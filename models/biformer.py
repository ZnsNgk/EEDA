...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
                       ...
                       side_dwconv=5, before_attn_dwconv=3, pre_norm=True, auto_pad=False, use_eda=False, num_patches=None):
        super().__init__()
    ...
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        for i in range(4):
            ...
            for j in range(depth[i]):
                eda = False
                if use_eda:
                    if i > 1:# and j>depth[i]//2-1:
                        eda = True
                stage.append(Block(dim=embed_dim[i], drop_path=dp_rates[cur + j], 
...
@register_model
def biformer_eeda_base(pretrained=False, pretrained_cfg=None,
                  pretrained_cfg_overlay=None, **kwargs):
    model = BiFormer(
        depth=[4, 4, 18, 4],
        embed_dim=[96, 192, 384, 768], mlp_ratios=[3, 3, 3, 3],

...
class GroupAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = x.reshape(B, h_group, self.ws, w_group, self.ws, C).transpose(2, 3).contiguous()

        qkv = self.qkv(x).reshape(B, total_groups, -1, 3, self.num_heads, C // self.num_heads).permute(3, 0, 1, 4, 2, 5).contiguous()
        ...
        attn = (attn @ v).transpose(2, 3).reshape(B, h_group, w_group, self.ws, self.ws, C).contiguous()
        x = attn.transpose(2, 3).reshape(B, N, C).contiguous()
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, use_eda=False, num_patches=None):
        super().__init__()
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], block_cls=Block, use_eda=False, **kwargs):
        super().__init__()
        ...
        for i in range(len(depths)):
            ...
            patch_num = self.patch_embeds[-1].num_patches + 1 if i == len(embed_dims) - 1 else self.patch_embeds[
                -1].num_patches
            self.num_patches.append(patch_num)
        ...
        for k in range(len(depths)):
            ...
            for i in range(depths[k]):
                eda = False
                if use_eda:
                    if k > 1 and i>depths[k]//2-1:
                        eda = True
                if k == len(depths) - 1 and i == 0:
                    self.num_patches[k] -= 1
                _block.append(block_cls(
                    dim=embed_dims[k], num_heads=num_heads[k], mlp_ratio=mlp_ratios[k], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
                    sr_ratio=sr_ratios[k], use_eda=eda, num_patches=self.num_patches[k]))
...
class PCPVT(CPVTV2):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 ...
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[4, 4, 4], sr_ratios=[4, 2, 1], block_cls=SBlock, use_eda=False, **kwargs):
        super(PCPVT, self).__init__(img_size, patch_size, in_chans, num_classes, embed_dims, num_heads,
                                    ...
                                    norm_layer, depths, sr_ratios, block_cls, use_eda)
...
@register_model
def pcpvt_eeda_base_v0(pretrained=False, **kwargs):
    model = CPVTV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1], use_eda=True,
        **kwargs)
    model.default_cfg = _cfg()
    return model

if __name__ == "__main__":
    model = pcpvt_eeda_base_v0().cuda()
    # print(model)
